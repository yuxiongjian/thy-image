...
#netLevel=2
#conv_num_output_s = [64,128]
#conv_ksize_s= [(5,5),(2,2)]
#conv_stride_s=[(3,3),(1,1)]
#pool_ksize_s=[(4,4),(2,2)]
#pool_stride_s=[(1,1),(1,1)]
#epochs = 40
#batch_size = 4096
...
netLevel= 3
fullLevel= 3
conv_num_output_s = [16,64,512,1024,2048]
conv_ksize_s=[ (5,5),(5,5),(3,3),(3,3),(3,3)]
conv_stride_s=[(1,1)]*netLevel
pool_ksize_s=[(2,2)]*netLevel
pool_stride_s=[(2,2)]*netLevel
epochs = 40
batch_size = 16
keep_probability = 0.7
stddev =  0.1
...
def normalize(x):
    ...
    return np.reshape(X_scaled, x.shape)
...
def one_hot_encode(x):
    ...
    x_labels = encode.transform(np.array(x).reshape(-1,1))
   

    # Change to float32, so it can be multiplied against the features in TensorFlow, which are float32
    
    return x_labels.toarray()
...
def conv2d_maxpool(x_tensor, conv_num_outputs, conv_ksize, conv_strides, pool_ksize, pool_strides):
    ...
    weight = tf.Variable(tf.truncated_normal([conv_ksize[0],conv_ksize[1],
                                              x_tensor.get_shape().as_list()[-1],conv_num_outputs],stddev = stddev))
...
def conv_net(x, keep_prob):
    ...
    tf_run = x
    for i in range(0,netLevel):
        tf_cv = conv2d_maxpool(tf_run, conv_num_output_s[i], conv_ksize_s[i],conv_stride_s[i],pool_ksize_s[i],pool_stride_s[i])
        tf_run = tf_cv
    
    
    tf_flatten = flatten(tf_run)
    
    for i in range(0,fullLevel):
        output5 = fully_conn(tf_flatten, 384)
        output6 = tf.nn.dropout(output5, keep_prob)  
        tf_flatten = output6
    
    
    logits_output = output(tf_flatten, 10)
    # TODO: return output
